import React, { useState } from "react";
import { useTodo } from "../Contexts/TodoContext";

export default function TodoItem({ todo }) {
    const [isTodoEditable, setIsTodoEditable] = useState(false);
    const [todoMsg, setTodoMsg] = useState(todo.todo);

    const { UpdateTodo, DeleteTodo, CompletedTodo } = useTodo();

    const editTodo = () => {
        UpdateTodo(todo.id, { ...todo, todo: todoMsg });
        setIsTodoEditable(false);
    }

    const toggleTodo = () => {
        CompletedTodo(todo.id);
    }

    const todoDelete = () => {
        DeleteTodo(todo.id);
    }
    return (
        <>
            <div className={`flex border border-black/10 rounded-lg px-3 py-1.5 shadow-sm shadow-white/50 duration-300 text-black ${todo.completed ? "bg-[#c6e9a7]" : "bg-[#ccbed7]"} gap-3`}>
                <input
                    type="checkbox"
                    className="cursor-pointer"
                    checked={todo.comeleted}
                    onChange={toggleTodo}
                />
                <input
                    type="text"
                    className={`border outline-none w-full bg-transparent rounded-lg ${
                        isTodoEditable ? "border-black/10 px-2" : "border-transparent"
                    } ${todo.completed ? "line-through" : ""}`}
                    onChange={(e) => setTodoMsg(e.target.value)}
                    value={todoMsg}
                />
                <button
                    className="inline-flex w-8 h-8 rounded-lg text-sm border border-black/10 justify-center items-center bg-gray-50 hover:bg-gray-100 shrink-0 disabled:opacity-50 cursor-pointer"
                    onClick={() => {
                        if (todo.comeleted) return;

                        if (isTodoEditable) {
                            editTodo();
                        } else setIsTodoEditable((prev) => !prev)

                    }}
                    disabled={todo.completed}
                >
                    {isTodoEditable ? '📁' : '✏️'}
                </button>
                <button onClick={todoDelete}
                    className="inline-flex w-8 h-8 rounded-lg text-sm border border-black/10 justify-center items-center bg-gray-50 hover:bg-gray-100 shrink-0"
                >❌</button>
            </div>
        </>
    )
}